...
> **v0.5.0 起的能力演进**：
> - **v0.5.0**：改用 WorkBuddy 稳定 DOM 锚点（`#root` / `.teams-container` / `[data-view-id]`）与原生 `--cb-*` 设计令牌做全局换色，取代 v0.4.x"面积阈值扫描 + 逐个元素内联强制"的脆弱兜底；新增应用内切换背景菜单：随时切换背景、上传自定义图片（Canvas 自动取色）、一键还原原生界面。
...
> - **v0.5.6**：修复"切换原生浅色界面无效、依旧全黑"——`applyMode` 现在除切 class 外会显式同步 `html[data-theme]`（浅色→`light` / 深色→`dark`）。WorkBuddy 表面令牌 `--wb-home-bg-primary` 由 `html[data-theme]` 驱动，旧版只切 class 没动 `data-theme`，导致 `<html>` 残留的 `data-theme="dark"` 让深色规则始终胜出；修复后原生浅色界面真正变浅且不回退。同期修复 CLI 换图被菜单 persistence 覆盖的问题，新增 `--preserve-state`（daemon 用）保证手动 `inject --image` 新图立即生效，而守护重启仍尊重用户上次选择的原生模式/自定义上传。
...
> 按钮挂载在 React 受控的顶部功能栏；v0.5.3 起通过 `MutationObserver` 自愈挂载，切换原生浅色/深色等触发重渲染时按钮会自动挂回，不会消失。
